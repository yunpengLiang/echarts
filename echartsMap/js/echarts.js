		...
		*/
		...
		function loadMap(mapCode, mapName) {
		    $.getJSON('china-main-city/' + mapCode + '.json', function (data) {
			     if (data) {
			     	//定义一个dataMapValue存储指定位置的数据
			     	var dataMapValue = [];
			     	if(mapCode!=100000){
			     		var nameList = [];
			     		for(var i=0;i<data.features.length;i++){
			     			nameList.push(data.features[i].properties.name);
			     		}
			     		for(var j=0;j<nameList.length;j++){
				     		for(var i=0;i<dataValue.length;i++){
			     				if(!nameList[j].indexOf(dataValue[i].name)){
				     				dataMapValue.push({name: dataValue[i].name, value: dataValue[i].value});
				     				break;
				     			}
				     		}
			     		}
			     	}else{
			     		dataMapValue = dataValue;
			     	}
